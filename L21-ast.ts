...
;;         |  ( for <var-decl> <num-exp> <num-exp> <cexp>)            / ForExp(v:VarDecl, start: NumExp, end: NumExp, body: Cexp))
...
export const isCompoundExp = (x: any): x is CompoundExp =>
    isAppExp(x) || isIfExp(x) || isForExp(x) ||isProcExp(x);
...
const parseForExp = (v : Sexp, params: Sexp[]): Result<ForExp> =>
    (params.length !== 3) ? makeFailure("Expression not of the form (for <VarDecl> <NumExp> <NumExp> <CExp>") :
        bind(mapResult(parseL21CExp, params),
        (cexps: CExp[]) => (isIdentifier(v)  && isNumExp(cexps[0]) && isNumExp(cexps[1]) ? (
                                       ((cexps[0].val > cexps[1].val) || isEmpty(cexps[2])) ? makeFailure("Invalid ForExp"):
